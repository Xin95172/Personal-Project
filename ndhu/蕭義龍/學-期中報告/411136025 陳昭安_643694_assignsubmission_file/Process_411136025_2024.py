...
# 1. 設定 2024 年路徑 (根據截圖，檔案已解壓縮為 CSV)
folder_name = r"C:\Users\chen2\Downloads\Option_2024\Option_2024"
file_pattern = os.path.join(folder_name, '*.csv') # 改為搜尋 .csv
file_list = glob.glob(file_pattern)

# 2. 設定 2024 年基本參數 (IV 計算用)
# 2024 年台股大盤指數多次突破兩萬點，設定 20000 為參考平均水位
DEFAULT_S = 20000.0  
...
print(f"🚀 開始執行 2024 年數據提煉... 總共 {len(file_list)} 個 CSV 檔案")

if len(file_list) == 0:
    print(f"❌ 錯誤：在 {folder_name} 找不到 CSV 檔案！請確認路徑或檔案類型。")
else:
    ...
    if daily_summary:
        ...
        final_df.to_csv("2024_Final_Report_Data.csv", index=False)
        print("\n🎉 2024 年數據提煉完成！檔案已存為 '2024_Final_Report_Data.csv'")
        print(final_df.head())