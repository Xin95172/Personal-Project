...
def main():
            ...
            with zipfile.ZipFile(zip_path, 'r') as z:
                ...
                for csv_name in z.namelist():
                    ...
                    if not df_day.empty:
                        ...
                        lookup['IV'] = lookup.apply(lambda x: find_iv(x['Price'], S0, x['StrikePrice'], T_days/252, Rf), axis=1)
                        df_day = df_day.merge(lookup, on=['StrikePrice', 'Price'], how='left')
                        df_day['TradeDate'] = trade_date
                        all_data_list.append(df_day)
                        print(f"    ✅ 成功計算 IV ({trade_date})")
    ...
    if all_data_list:
        ...
        # 存檔
        final_df.to_csv(os.path.join(base_path, 'TXO_ZIP_Process_Detailed.csv'), index=False, encoding='utf-8-sig')
        summary.to_csv(os.path.join(base_path, 'TXO_ZIP_Process_Summary.csv'), encoding='utf-8-sig')
