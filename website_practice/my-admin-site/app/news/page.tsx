...
export default async function NewsPage() {
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      const response = await withTimeout(supabase.from("articles").select("id, title, excerpt, content, author_name, created_at").eq("status", "published").order("created_at", { ascending: false }));
