...
export default async function QaPage() {
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      const response = await withTimeout(supabase.from("question_submissions").select("id, question, answer, answered_at").eq("status", "answered").not("answer", "is", null).order("answered_at", { ascending: false }).limit(20));
