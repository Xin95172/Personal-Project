...
@{ configuration=$Configuration; mode=$mode; sharedTimeline=[bool]$SharedTimeline; sourceManifestSha256=$sourceManifestHash; kit=$version; files=$hashes; signed=$false } | ConvertTo-Json -Depth 4 | Set-Content "$package\manifest.json"
